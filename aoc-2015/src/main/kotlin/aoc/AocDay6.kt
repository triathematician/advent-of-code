...
class AocDay6: AocDay(6) {
    ...
    data class Toggle(val op: ToggleOp, val corner1: Loc, val corner2: Loc) {
        ...
        fun applyTo(grid: Array<Array<Int>>) {
            pos.forEach {
                when (op) {
        ...
        fun applyTo2(grid: Array<Array<Int>>) {
            pos.forEach {
                when (op) {
    ...
    override fun calc1(input: String) = input.lines().map { parseToggleInstruction(it) }.let {
        val grid = Array(1000) { Array(1000) { 0 } }
        it.forEach { it.applyTo(grid) }
        grid.sumOf { it.sumOf { it }}
    }
    override fun calc2(input: String) = input.lines().map { parseToggleInstruction(it) }.let {
        val grid = Array(1000) { Array(1000) { 0 } }
